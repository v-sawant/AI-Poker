...
class pokerGames(object):
    ...
    def getHandCategory(self):
        #print("XXXXXXXXX", self.hand_dict['name'])
        return self.hand_dict['name']

h_eval = HandEvaluator()
...
def queryOpenAction(_minimumPotAfterOpen, _playersCurrentBet, _playersRemainingChips):
    ...
    def chooseOpenCheckAllin():
        t_hand_cat_name = infoAgent.getHandCategory()
        print("t_hand_cat_name~~~~~~~~~~~~~~", t_hand_cat_name)
        #Bluff on open action
        if(t_hand_cat_name == HandRankCategory.STRAIGHT_FLUSH
        or t_hand_cat_name == HandRankCategory.FOUR_OF_A_KIND): 
            return ClientBase.BettingAnswer.ACTION_ALLIN
        elif(t_hand_cat_name == HandRankCategory.FULL_HOUSE
        or t_hand_cat_name == HandRankCategory.FLUSH
        or t_hand_cat_name == HandRankCategory.STRAIGHT):
            return ClientBase.BettingAnswer.ACTION_OPEN,  (random.randint(0, 10) + _minimumPotAfterOpen) if _playersCurrentBet + _playersRemainingChips > (10 + _minimumPotAfterOpen) else ClientBase.BettingAnswer.ACTION_OPEN, (_minimumPotAfterOpen)
        elif(t_hand_cat_name == HandRankCategory.THREE_OF_A_KIND
        or t_hand_cat_name == HandRankCategory.TWO_PAIR):
            return ClientBase.BettingAnswer.ACTION_OPEN,  (random.randint(7, 15) + _minimumPotAfterOpen) if  _playersCurrentBet + _playersRemainingChips > (15 + _minimumPotAfterOpen) else ClientBase.BettingAnswer.ACTION_OPEN, (_minimumPotAfterOpen)
        elif(t_hand_cat_name == HandRankCategory.ONE_PAIR):
            return ClientBase.BettingAnswer.ACTION_OPEN,  (random.randint(12, 20) + _minimumPotAfterOpen) if  _playersCurrentBet + _playersRemainingChips > (20 + _minimumPotAfterOpen) else ClientBase.BettingAnswer.ACTION_OPEN, (_minimumPotAfterOpen)
        else:
            return ClientBase.BettingAnswer.ACTION_OPEN,  (random.randint(0, 2) + _minimumPotAfterOpen) if  _playersCurrentBet + _playersRemainingChips > (20 + _minimumPotAfterOpen) else ClientBase.BettingAnswer.ACTION_CHECK
    
    return chooseOpenCheckAllin()
...
def queryCallRaiseAction(_maximumBet, _minimumAmountToRaiseTo, _playersCurrentBet, _playersRemainingChips):
    ...
    def chooseCallRaiseFoldAllin():
        t_hand_cat_name = infoAgent.getHandCategory()
        print("t_hand_cat_name------------", t_hand_cat_name)
        #Bluff on open action
        if(t_hand_cat_name == HandRankCategory.STRAIGHT_FLUSH
        or t_hand_cat_name == HandRankCategory.FOUR_OF_A_KIND): 
            return ClientBase.BettingAnswer.ACTION_RAISE,  (random.randint(12, 20) + _minimumAmountToRaiseTo) if _playersCurrentBet+ _playersRemainingChips + 20 > _minimumAmountToRaiseTo else ClientBase.BettingAnswer.ACTION_RAISE, _minimumAmountToRaiseTo
        elif(t_hand_cat_name == HandRankCategory.FULL_HOUSE
        or t_hand_cat_name == HandRankCategory.FLUSH
        or t_hand_cat_name == HandRankCategory.STRAIGHT):
            return (ClientBase.BettingAnswer.ACTION_RAISE, (random.randint(12, 16) + _minimumAmountToRaiseTo)) if _playersCurrentBet+ _playersRemainingChips + 16 > _minimumAmountToRaiseTo else (ClientBase.BettingAnswer.ACTION_RAISE, _minimumAmountToRaiseTo)
        elif(t_hand_cat_name == HandRankCategory.THREE_OF_A_KIND
        or t_hand_cat_name == HandRankCategory.TWO_PAIR):
            return ClientBase.BettingAnswer.ACTION_RAISE,  (random.randint(5, 15) + _minimumAmountToRaiseTo) if _playersCurrentBet+ _playersRemainingChips + 15 > _minimumAmountToRaiseTo else ClientBase.BettingAnswer.ACTION_RAISE, _minimumAmountToRaiseTo
        elif(t_hand_cat_name == HandRankCategory.ONE_PAIR):
            return ClientBase.BettingAnswer.ACTION_RAISE,  (random.randint(0, 5) + _minimumAmountToRaiseTo) if _playersCurrentBet+ _playersRemainingChips + 5 > _minimumAmountToRaiseTo else ClientBase.BettingAnswer.ACTION_RAISE, _minimumAmountToRaiseTo
        else:
            if _maximumBet > (_playersCurrentBet + 5):
            ...
            elif _playersRemainingChips > 180:
                return ClientBase.BettingAnswer.ACTION_FOLD
            else:
                 return {
                        0: ClientBase.BettingAnswer.ACTION_CALL if _playersCurrentBet+ _playersRemainingChips > _maximumBet else ClientBase.BettingAnswer.ACTION_FOLD,
                        1: ClientBase.BettingAnswer.ACTION_FOLD
                        }.get(random.randint(0, 2), ClientBase.BettingAnswer.ACTION_ALLIN)
    
    return chooseCallRaiseFoldAllin()
...
def queryCardsToThrow(_hand):
    ...
    t_hand_cat_name = infoAgent.getHandCategory()
    #print("t_hand_cat_name~~~~~~~~~~~~~~", t_hand_cat_name)
    t_hand_dict = infoAgent.hand_dict
    
    def sortByRank(_hand):
        card_list = []#list of tuple
        card_list = sorted(_hand, key=operator.itemgetter(0,1))
        #print("Sorted By Rank...\n", card_list)
        return card_list

    _hand_rsorted = sortByRank(_hand)
    if(t_hand_cat_name == HandRankCategory.FOUR_OF_A_KIND):
        if (t_hand_dict['rank5'] == 2 or 
        t_hand_dict['rank5'] == 3 or 
        t_hand_dict['rank5'] == 4 or 
        t_hand_dict['rank5'] == 5 or 
        t_hand_dict['rank5'] == 6):
            throw_cards += _hand_rsorted[0] + ' '

    elif(t_hand_cat_name == HandRankCategory.THREE_OF_A_KIND):
        if (t_hand_dict['rank4'] == 2 or 
        t_hand_dict['rank4'] == 3 or 
        t_hand_dict['rank4'] == 4 or 
        t_hand_dict['rank4'] == 5 or 
        t_hand_dict['rank4'] == 6 or 
        t_hand_dict['rank4'] == 7 or 
        t_hand_dict['rank4'] == 8 or 
        t_hand_dict['rank4'] == 9):
            throw_cards += _hand_rsorted[1] + ' '
            throw_cards += _hand_rsorted[0] + ' '
        elif (t_hand_dict['rank5'] == 2 or 
        t_hand_dict['rank5'] == 3 or 
        t_hand_dict['rank5'] == 4 or 
        t_hand_dict['rank5'] == 5 or 
        t_hand_dict['rank5'] == 6 or 
        t_hand_dict['rank5'] == 7 or 
        t_hand_dict['rank5'] == 8 or 
        t_hand_dict['rank5'] == 9):
            throw_cards += _hand_rsorted[0] + ' '

    elif(t_hand_cat_name == HandRankCategory.TWO_PAIR):
        throw_cards += _hand_rsorted[0] + ' '
    
    elif(t_hand_cat_name == HandRankCategory.ONE_PAIR):
        throw_cards += _hand_rsorted[0] + ' '
        throw_cards += _hand_rsorted[1] + ' '
        throw_cards += _hand_rsorted[2] + ' '
    
    elif(t_hand_cat_name == HandRankCategory.HIGH):
        if (t_hand_dict['rank3'] == 2 or 
        t_hand_dict['rank3'] == 3 or 
        t_hand_dict['rank3'] == 4 or 
        t_hand_dict['rank3'] == 5 or 
        t_hand_dict['rank3'] == 6 or 
        t_hand_dict['rank3'] == 7 or 
        t_hand_dict['rank3'] == 8 or 
        t_hand_dict['rank3'] == 9):
            throw_cards += _hand_rsorted[2] + ' '
            throw_cards += _hand_rsorted[1] + ' '
            throw_cards += _hand_rsorted[0] + ' '
        elif (t_hand_dict['rank4'] == 2 or 
        t_hand_dict['rank4'] == 3 or 
        t_hand_dict['rank4'] == 4 or 
        t_hand_dict['rank4'] == 5 or 
        t_hand_dict['rank4'] == 6 or 
...
def infoPlayerHand(_playerName, _hand):
    ...
    infoAgent.hand_wt = h_eval.evaluate(_hand[:5])
    print("evaluated_hand_val=========", infoAgent.hand_wt)
    infoAgent.hand_dict = h_eval.getHandInfo()
    #print("player hand dicti:::::::::::", infoAgent.hand_dict )
